...
(function(){
  ...
  function renderHealthView(){
    ...
    if(lanes)lanes.innerHTML=rows.length?rows.slice(0,6).map(function(row){var tone=row.score<60?'bad':row.score<85?'warn':'good';return '<button type="button" class="rs-health-lane '+tone+'" data-rs-channel="'+row.channel+'" aria-label="Tune channel '+row.channel+' '+htmlSafe(row.name)+'"><span class="rs-health-lane-id">'+String(row.channel).padStart(3,'0')+'</span><span class="rs-health-lane-name"><b>'+htmlSafe(row.name)+'</b><small>'+htmlSafe(row.cat||'LOCAL')+' · '+(row.frames?row.frames+' frames':'no frame yet')+'</small></span><span class="rs-health-lane-metrics"><b>'+row.score+'</b><small>'+[row.frameP50!=null?'F '+formatMs(row.frameP50):'F —',row.switchP50!=null?'S '+formatMs(row.switchP50):'S —','Q '+(row.queue==null?'—':row.queue),row.failures?'ERR '+row.failures:'OK'].join(' · ')+'</small></span></button>';}).join(''):'<div class="rs-health-empty">Tune channels to build a local scorecard. Weak lanes will appear here automatically.</div>';
  }
