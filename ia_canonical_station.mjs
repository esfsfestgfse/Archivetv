...
export const IA_CANONICAL_PILOT_PROFILES = Object.freeze({
  ...
  "classic-cartoons": Object.freeze({
    ...
    includeAny: Object.freeze(["cartoon", "cartoons", "animated", "animation", "looney tunes", "popeye", "bugs bunny", "tom and jerry", "fleischer", "woody woodpecker", "theatrical cartoon", "hanna barbera", "filmation", "jay ward", "space ghost", "scooby doo"]),
  ...
  "game-shows": Object.freeze({
    ...
    includeAny: Object.freeze(["game show", "game-show", "quiz show", "quiz-show", "panel show", "panel-show", "match game", "jeopardy", "price is right", "family feud", "password", "pyramid", "newlywed game", "hollywood squares", "let's make a deal", "press your luck", "concentration", "to tell the truth", "what's my line", "beat the clock", "the $64,000 question", "wheel of fortune"]),
    titleAny: Object.freeze([]),
...
export function buildCanonicalManifest(profile, rawItems, generatedAt = new Date().toISOString()) {
  ...
  return {
    ...
    rules: {
      ...
      requiredDecades: profile.requiredDecades,
      searchQueries: profile.searchQueries,
      includeAny: profile.includeAny,
      excludeAny: profile.excludeAny,
    ...
    verified: selected.length >= profile.minCatalog
      && Object.values(Object.fromEntries([...decadeCounts.entries()])).filter((value) => Number(value) > 0).length >= Number(profile.minDecades || 0)
      && (profile.requiredDecades || []).every((decade) => Number(decadeCounts.get(Number(decade)) || 0) > 0)
      && (selected.length ? Math.max(...decadeCounts.values()) / selected.length : 0) <= Number(profile.maxDecadeShare || 1),
    items: selected,
