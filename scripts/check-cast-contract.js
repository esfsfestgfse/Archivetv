...
for (const file of ['the_dial_desktop.html', 'the_dial_mobile.html']) {
  ...
  for (const [token, reason] of [
    ...
    ['.021-cast-tune-sync', 'must carry the current Cast tune-synchronization build stamp'],
...
for (const token of ['cast_receiver_framework.js', 'addCustomMessageListener', 'REALSIGNAL_STATE', 'REALSIGNAL_MEDIA', 'desiredChannel', 'cast-media-player', 'getPlayerManager', 'mediaContentType', 'castReceiver']) if (!receiver.includes(token)) issues.push(`receiver: missing ${token}`);
