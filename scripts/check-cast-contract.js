...
for (const file of ['the_dial_desktop.html', 'the_dial_mobile.html']) {
  ...
  for (const [token, reason] of [
    ...
    ['.022-cast-guide-handshake', 'must carry the current Cast guide-handshake build stamp'],
...
for (const token of ['cast_receiver_framework.js', 'addCustomMessageListener', 'REALSIGNAL_STATE', 'REALSIGNAL_MEDIA', 'REALSIGNAL_GUIDE_READY', 'desiredChannel', 'cast-media-player', 'getPlayerManager', 'mediaContentType', 'castReceiver']) if (!receiver.includes(token)) issues.push(`receiver: missing ${token}`);
