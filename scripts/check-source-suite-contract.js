...
if (!serverCatalog.includes('if (raw.length < SOURCE_MIN_READY)') || !serverCatalog.includes('SOURCE_QUERY_WINDOW = 4') || !serverCatalog.includes('SOURCE_MAX_QUERY_WINDOW = 6') || !serverCatalog.includes('profile.queryWindow || SOURCE_QUERY_WINDOW') || !serverCatalog.includes('youtubeSearchDuration(profile, rotation)') || !serverCatalog.includes('Full television/film/performance lanes are') || !serverCatalog.includes('SOURCE_YOUTUBE_QUERY_CONCURRENCY = 2') || !serverCatalog.includes('Math.ceil(ids.length / 50)') || !serverCatalog.includes('SOURCE_PROVIDER_BUDGET_MS = 4500') || !serverCatalog.includes('withTimeout(task, SOURCE_PROVIDER_BUDGET_MS)')) issues.push('Source discovery must rotate a bounded query window, keep full-program lanes in the long-form bucket, allow only shallow lanes a capped expansion, bound search concurrency, enforce a provider admission budget, and chunk detail hydration to 50 IDs');
...
for (const key of ['print-shop', 'screen-test', 'sound-lab', 'stage-door', 'travel-reel', 'variety-hour', 'western-screen']) {
  const start = sourceRegistry.indexOf(`"${key}":`);
  const next = sourceRegistry.indexOf('\n  },', start);
  const section = sourceRegistry.slice(start, next >= 0 ? next + 5 : sourceRegistry.length);
  if (!section.includes('"queryLimit": 12')) issues.push(`${key} must retain the expanded query pool for serial source rotations`);
