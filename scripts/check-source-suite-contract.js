...
if (!serverCatalog.includes('if (raw.length < SOURCE_MIN_READY)') || !serverCatalog.includes('SOURCE_QUERY_WINDOW = 4') || !serverCatalog.includes('SOURCE_MAX_QUERY_WINDOW = 6') || !serverCatalog.includes('profile.queryWindow || SOURCE_QUERY_WINDOW') || !serverCatalog.includes('youtubeSearchDuration(rotation)') || !serverCatalog.includes('SOURCE_YOUTUBE_QUERY_CONCURRENCY = 2') || !serverCatalog.includes('Math.ceil(ids.length / 50)')) issues.push('Source discovery must rotate a bounded query window, allow only shallow lanes a capped expansion, alternate eligible YouTube duration buckets, bound search concurrency, and chunk detail hydration to 50 IDs');
...
for (const key of ['family-tv-club', 'garden-ledger', 'green-culture', 'lesson-reel', 'local-signal', 'memory-bank', 'newsreel-exchange', 'print-shop', 'screen-test', 'sound-lab', 'stage-door', 'travel-reel', 'variety-hour', 'western-screen']) {
