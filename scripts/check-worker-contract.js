...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v64"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v64"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/const laneLimit = firstApprovedLane/.test(source) || !/Math\.min\(IA_FOREGROUND_DISCOVERY_LANES, queries\.length\)/.test(source) || !/const lanePromises = queries\.slice\(0, laneLimit\)/.test(source) || !/Promise\.allSettled\(lanePromises\)/.test(source)) issues.push('cold queue discovery must use one foreground Archive rail and defer every other rail');
