...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v56"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v56"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/let sharedFallback = null/.test(source) || !/items: lastGood\.items\.slice\(offset\)\.concat\(lastGood\.items\.slice\(0, offset\)\)/.test(source) || !/program-director-last-good/.test(source)) issues.push('a partial shared queue must hand off to a rotated full last-good shelf while refill continues');
if (!/const underfilledLastGood = await sharedQueueGet\(env, lastGoodKey\)/.test(source) || !/hydrated\.ready < count/.test(source) || !/items: underfilledLastGood\.items\.slice\(offset\)\.concat\(underfilledLastGood\.items\.slice\(0, offset\)\)/.test(source)) issues.push('an underfilled fresh queue must retain the complete last-good buffer while refill continues');
...
if (!/async function expandSeedArchiveContainers\(/.test(source) || !/item\.media && item\.media\.url/.test(source) || !/payload\.emergency === true/.test(source) || !/const seedEpisodes = await expandSeedArchiveContainers\(/.test(source) || !/slice\(0, 2\)\.forEach\(\(episode\)/.test(source)) issues.push('background expansion must use the exact foreground container, prioritize discovery after fallback, skip direct-ready fallbacks, and cap siblings per parent');
