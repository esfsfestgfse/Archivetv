...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v68"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v68"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/const laneLimit = firstApprovedLane/.test(source) || !/Math\.min\(IA_FOREGROUND_DISCOVERY_LANES, searchQueries\.length\)/.test(source) || !/const lanePromises = searchQueries\.slice\(0, laneLimit\)/.test(source) || !/Promise\.allSettled\(lanePromises\)/.test(source)) issues.push('cold queue discovery must use one foreground Archive rail and defer every other rail');
...
if (!/candidateLimit = Math\.max\(count, Math\.min\(IA_STRICT_CATALOG_CANDIDATE_MAX/.test(source) || !/const expandedSources = new Set\(expanded\.map/.test(source) || !/const approvedPrograms = approved\.filter/.test(source) || !/const expansionSeeds = firstApprovedLane \? \[\] : hintedSeeds\.concat\(genericSeeds\)/.test(source) || !/const expandedParents = new Set\(\)/.test(source) || !/expandedParents\.has\(String\(doc\.identifier/.test(source) || !/candidateItems: items\.slice\(0, candidateLimit\)/.test(source) || !/const emergencyDepth = Math\.min\(candidateCount, Math\.max\(count, 8\)\)/.test(source) || !/const cachedShelf =/.test(source) || !/const sharedShelf =/.test(source) || !/Number\(rotation\) \|\| 0\) \* requested/.test(source)) issues.push('IA queue builder must retain a deep candidate shelf, widen sparse emergency lanes, rotate cached/shared shelves by full shelf windows, drop expanded parent indexes, and keep manifest expansion off the cold path');
...
if (!/IA_FOREGROUND_DISCOVERY_LANES\s*=\s*1/.test(source) || !/const fastQueries = orderedQueries\.slice\(0, Math\.min\(IA_FOREGROUND_DISCOVERY_LANES, orderedQueries\.length\)\)/.test(source)) issues.push('cold queue discovery must use one foreground Archive rail and defer the rest');
