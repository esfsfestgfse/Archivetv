...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v63"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v63"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/const videoByEpisode = new Map\(\)/.test(source) || !/archiveEpisodeKey\(file\)/.test(source) || !/videoByEpisode\.set\(key, file\)/.test(source) || !/runtime: file\.duration \|\| file\.runtime \|\| doc\.runtime/.test(source)) issues.push('complete-series expansion must collapse duplicate encodes without treating byte size as duration');
if (!/archiveEpisodeRotation\(\[\.\.\.videoByEpisode\.values\(\)\], rotation, salt\)/.test(source) || !/underCap\("source", keys\.source, 2\)/.test(source)) issues.push('expanded episodes must rotate through a series without letting one parent occupy the whole shelf');
...
if (!/const minimumFiles = archiveContainerHint\(doc\) \? 2 : 3/.test(source) || !/const genericSeeds = catalogWantsVideo \? approved\.filter\(\(doc\) => !archiveContainerHint\(doc\)\)\.slice\(0, 1\) : \[\]/.test(source) || !/firstApprovedLane && approved\.length/.test(source)) issues.push('multi-file Archive items must be probed even when their title lacks a collection label');
