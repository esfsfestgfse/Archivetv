...
if (!/const warmLastGood = await sharedQueueGet\(env, lastGoodKey\)/.test(source) || !/Number\(warmLastGood\.rotation \|\| 0\) === rotation/.test(source) || !/program-director-warm-start/.test(source)) issues.push('same-rotation last-good shelves must provide a warm start before cold Archive discovery');
...
if (!/if \(hydrated\.ready > 0\) sharedQueuePut\(env, sharedKey, hydrated, IA_PARTIAL_QUEUE_TTL_SECONDS, ctx\)/.test(source)) issues.push('first verified program must seed the durable last-good shelf before background hydration completes');
