...
if (!/candidateLimit = Math\.max\(count, Math\.min\(30, Number\(count\) \|\| 5\)\)/.test(source) || !/const expandedSources = new Set\(expanded\.map/.test(source) || !/const approvedPrograms = approved\.filter/.test(source) || !/candidateItems: items\.slice\(0, candidateLimit\)/.test(source)) issues.push('IA queue builder must retain a deep candidate shelf and drop expanded parent indexes');
