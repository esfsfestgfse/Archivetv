...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v57"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v57"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/candidateLimit = Math\.max\(count, Math\.min\(30, Number\(count\) \|\| 5\)\)/.test(source) || !/const expandedSources = new Set\(expanded\.map/.test(source) || !/const approvedPrograms = approved\.filter/.test(source) || !/const expansionSeeds = firstApprovedLane \? \[\] : hintedSeeds\.concat\(genericSeeds\)/.test(source) || !/const expandedParents = new Set\(\)/.test(source) || !/expandedParents\.has\(String\(doc\.identifier/.test(source) || !/candidateItems: items\.slice\(0, candidateLimit\)/.test(source)) issues.push('IA queue builder must retain a deep candidate shelf, drop expanded parent indexes, and keep manifest expansion off the cold path');
