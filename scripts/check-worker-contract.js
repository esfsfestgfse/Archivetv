...
if (!/IA_FOREGROUND_CONTAINER_EXPANSIONS\s*=\s*0/.test(source) || !/IA_BACKGROUND_CONTAINER_EXPANSIONS\s*=\s*2/.test(source) || !/const expansionLimit = firstApprovedLane/.test(source)) issues.push('foreground tuning must defer complete-series expansion and bound background container work');
...
if (!/candidateLimit = Math\.max\(count, Math\.min\(30, Number\(count\) \|\| 5\)\)/.test(source) || !/const expandedSources = new Set\(expanded\.map/.test(source) || !/const approvedPrograms = approved\.filter/.test(source) || !/const expansionSeeds = firstApprovedLane \? \[\] : hintedSeeds\.concat\(genericSeeds\)/.test(source) || !/const expandedParents = new Set\(\)/.test(source) || !/expandedParents\.has\(String\(doc\.identifier/.test(source) || !/candidateItems: items\.slice\(0, candidateLimit\)/.test(source) || !/const emergencyDepth = Math\.min\(candidateCount, Math\.max\(count, 8\)\)/.test(source) || !/const cachedShelf =/.test(source) || !/const sharedShelf =/.test(source) || !/Number\(rotation\) \|\| 0\) \* requested/.test(source)) issues.push('IA queue builder must retain a deep candidate shelf, widen sparse emergency lanes, rotate cached/shared shelves by full shelf windows, drop expanded parent indexes, and keep manifest expansion off the cold path');
