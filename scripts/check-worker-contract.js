...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v45"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v45"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/const chosen = requested \|\| \(wantsVideo \? video\[0\] : wantsAudio \? audio/.test(source) || !/queuePlayable\(item\.identifier, cacheOrigin, ctx, mediaTypes\)/.test(source)) issues.push('hydration must keep archive derivatives within the requested video or audio contract');
