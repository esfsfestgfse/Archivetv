...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v55"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v55"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/async function expandSeedArchiveContainers\(/.test(source) || !/item\.media && item\.media\.url/.test(source) || !/const seedEpisodes = await expandSeedArchiveContainers\(/.test(source) || !/slice\(0, 2\)\.forEach\(\(episode\)/.test(source)) issues.push('background expansion must use the exact foreground container, skip direct-ready fallbacks, and cap siblings per parent');
