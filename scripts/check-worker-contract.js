...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v79"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v79"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/const fullShelf = Number\(payload\.ready \|\| payload\.items\.length\) >= 5/.test(source) || !/if \(fullShelf\) \{[\s\S]*mergeIaFallbackCandidates\(previous, payload\)/.test(source)) issues.push('only a full five-show shelf may replace the durable last-good buffer');
...
if (!/const needsPlayableDepth = candidateItems\.length > readyItems\.length \|\| playableCandidates < Math\.min\(candidateCount, Math\.max\(count, 12\)\)/.test(source) || !/ready\.ready >= count && !needsPlayableDepth/.test(source)) issues.push('complete foreground shelves must hydrate their unused candidate catalog for future rotations');
