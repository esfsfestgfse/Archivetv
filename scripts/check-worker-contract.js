...
if (!/IA_QUEUE_CACHE_VERSION\s*=\s*"v58"/.test(source) || !/IA_LAST_GOOD_CACHE_VERSION\s*=\s*"v58"/.test(source)) issues.push('queue cache must invalidate stale exact and last-good shelves together');
...
if (!/Number\(shared\.ready\) > 0/.test(source) || !/shared\.partial && Array\.isArray\(shared\.candidateItems\)/.test(source) || !/scheduleCachedIaHydration\(shared, count/.test(source)) issues.push('shared IA shelves must serve a verified partial first item while background refill continues');
