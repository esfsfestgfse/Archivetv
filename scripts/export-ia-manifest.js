...
const manifest = vm.runInContext(`CH.filter(function(ch){return !ch.source;}).map(function(ch){
  ...
  return {channel:String(ch.num),name:ch.nm,queries:iaQueueQueries(sl),themeTerms:iaProgramThemeTerms(sl),denyTerms:iaProgramDenyTerms(sl),diversity:iaQueueDiversity(sl),mediaTypes:ch.audio?['audio']:['movies'],themeMinScore:Math.max(1,Math.min(12,Number(sl&&sl.program&&sl.program.themeMinScore)||1))};
})`, context);
