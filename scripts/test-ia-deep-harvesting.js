...
check(/IA_QUEUE_CACHE_VERSION\s*=\s*"v113"/.test(relay), 'deep harvest invalidates the prior queue namespace');
...
check(/IA_BACKGROUND_COLLECTION_EPISODES_PER_PARENT\s*=\s*16/.test(relay) && /IA_BACKGROUND_CONTAINER_EXPANSIONS\s*=\s*6/.test(relay), 'container harvesting covers multiple parents and episode positions');
...
check(/background\s*\?\s*\(iaDepthRecoveryEnabled\(channel\) \? 24 : 16\)/.test(relay), 'background rotations sample a deep deterministic Archive page window');
...
check(/IA_DEPTH_PLAYABLE_TARGET\s*=\s*48/.test(relay) && /IA_BACKGROUND_PLAYABLE_TARGET\s*=\s*36/.test(relay), 'playable depth is measured separately from the five-item on-air shelf');
...
check(/"80": \[[\s\S]*Makingof1946[\s\S]*HuntingSeason/.test(relay), 'The Hunt has verified outdoors recovery media');
