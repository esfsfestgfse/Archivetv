...
async function test(file) {
  ...
  s.fetch=async()=>({ok:true,json:async()=>({items:[stale],ready:5,candidates:5,fallback:true,stale:true,rotation:4})});
  await vm.runInContext('refillIAQueue({num:12},{})',s);
  ...
  s.fetch=async()=>({ok:true,json:async()=>({items:[fresh],ready:5,candidates:5,rotation:4})});
  await vm.runInContext('refillIAQueue({num:12},{})',s);
