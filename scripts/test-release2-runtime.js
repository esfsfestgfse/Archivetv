...
for (const file of builds) {
  ...
  check(html.includes('currentDuration') && html.includes('guideItemRuntime(nextItem)'), `${file}: guide exposes Source Suite current/next runtime data`);
    check(html.includes('2.2.1-' + (file.includes('mobile') ? 'mobile' : 'desktop') + '.234-ia-rolling-freshness-longtail'), `${file}: build stamp is RealSignal 2.2.1 — IA Rolling Freshness Long-Tail`);
